  ...
  return (
    <>
      ...
      {selectedPoints.length >= 3 && isCreatingPolygon && (
        <Marker position={getPolygonCenter(selectedPoints)} icon={fireIcon}>
...
export default function TurkeyMap() {
  ...
  const fetchFireZones = async () => {
    try {
      ...
      console.log("Yangın bölgeleri verisi:", response.data);
      const points: FirePoint[] = [];
      const zones: FireZone[] = [];

      response.data.forEach((zone: any) => {
        if (zone.type === ALAN_TYPE.YANAN) {
          // Polygon alanları
          console.log("Polygon verisi:", zone);
          zones.push({
            name: zone.name,
        ...
        <div className="flex items-center gap-4">
          ...
          {isCreatingPolygon && (
            <div className="text-sm text-gray-600">
              ...
              <p className="mt-1">
                Haritada 4 nokta seçerek yanan alan polygonu oluşturun
              </p>
            </div>
          )}
        </div>
